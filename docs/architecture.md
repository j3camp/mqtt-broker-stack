...
`scripts/dynsec-command.sh` 啟動一次性、唯讀、drop capabilities 的管理容器。容器透過 TLS 連線至 `mosquitto-control:1884`，並由 Docker secret 讀取密碼。Compose file secret 在 broker 啟動時會由 root entrypoint 複製成僅 `mosquitto` 使用者可讀的暫存檔，主機 secret 仍維持 mode 600。broker 或 Docker socket 都不會掛載到管理容器。
